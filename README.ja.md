...
`setup.sh` が Node・claude・依存関係を点検し、足りないものは確認のうえインストールし、**ホーム配下で git リポジトリが集まっているフォルダーを探してプロジェクトフォルダーを選ばせ**、デスクトップのショートカットまで作ります。終わったら**ショートカットをダブルクリック**してください(Chrome があればアドレスバーのないアプリ風ウィンドウ、なければ既定のブラウザで開きます。または `npm start` して http://127.0.0.1:41730)。
...
## すでにインストール済みの方へ(アップデート)

**クローンし直す必要はありません。** 設定(`~/.config/project-launcher/`)とメモ(`~/.local/state/project-launcher/`)はリポジトリの外にあるため保持され、デスクトップのショートカットもそのまま使えます。

```bash
cd ~/claude-wsl-launcher   # インストールしたフォルダー
git pull
npm install
```
...
> ⚠️ **ここが一番重要です。** ダッシュボードを起動したまま `git pull` すると、画面(ボタン・モーダル)は新しいコードになりますが、**すでに動いているサーバーは古いコードのまま**なので、新機能を押すと「サーバーエラー」になります。故障ではなく、サーバーが古いだけです。
> ヘッダーの **[サーバー終了]** を押してから、デスクトップのショートカットを実行し直してください。

**GitHub からの取り込み・削除を使うには** gh CLI へのログインが一度だけ必要です(無くても他の機能は動きます):

```bash
gh auth login
```

うまくいったか確認するには `bash scripts/doctor.sh` を実行してください。
...
- **GitHub から取り込み**: ヘッダーの `⬇ 取り込み` で自分の GitHub リポジトリ一覧を検索してクローン(gh ログインが必要)。すでに取得済みのリポジトリは区別表示されます。GitHub の URL を直接貼り付けて取り込むこともできます。
- **プロジェクトの削除**: ローカルフォルダーをゴミ箱へ移動し、必要なら **GitHub リポジトリも同時に削除**(チェックボックス、gh ログインが必要)。実行中のプロジェクトとこのツール自身のフォルダーは削除できません。
- **メモ**: カードごとに 1 行のメモを付けて、どのプロジェクトか分かるようにします(`~/.local/state/project-launcher/notes.json` に保存)。
...
- **GitHub 公開/非公開バッジと切替**: origin が GitHub のカードに `owner/repo` と**公開/非公開**バッジ(gh ログイン時。無い場合は `?`)。バッジをクリックすると確認後に `gh repo edit --visibility` で切り替え。
...
- **プロジェクトフォルダーの選択・変更**: インストール時に候補から選び、ダッシュボード上部の [変更] でいつでも変更可能。カードが一つも無いときはバナーからも変更できます。優先順位: `PROJECTS_ROOT` 環境変数 > 設定ファイル > `~/projects`。
- **ランチャー自体の更新**: ヘッダーの `⟳ アップデート` で新しいバージョンを確認して取得(`git pull --ff-only` + `npm install`)し、反映のための終了まで案内します。手元の変更が更新対象のファイルと重なる場合や履歴が分岐している場合は**何も変更せず**理由を伝えます。
