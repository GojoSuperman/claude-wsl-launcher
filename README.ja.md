...
- **Claude Code などの AI エージェントにインストールを任せる場合**: `bash scripts/setup.sh --yes` — すべての質問に「はい」で進み、プロジェクトフォルダーはリポジトリが最も多い候補を自動選択します。ツール自体は `~/projects` のような**プロジェクトフォルダーの中ではなくホーム(`~`)に**置いてください。
