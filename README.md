...
**다시 클론할 필요 없습니다.** 설정(`~/.config/project-launcher/`)과 메모(`~/.local/state/project-launcher/`)는 저장소 바깥에 있어 그대로 유지되고, 바탕화면 단축키도 다시 만들 필요가 없습니다.

```bash
cd ~/claude-wsl-launcher   # 설치한 폴더
git pull
npm install
```
...
> ⚠️ **이게 제일 중요합니다.** 대시보드가 켜져 있는 채로 `git pull` 만 하면, 화면(버튼·모달)은 새 코드로 바뀌지만 **이미 실행 중인 서버는 옛 코드 그대로**라 새 기능을 누를 때 "서버 오류" 가 납니다. 고장이 아니라 서버가 옛날이라 그런 것입니다.
> 헤더의 **[서버 종료]** 를 누른 뒤 바탕화면 단축키를 다시 실행하면 해결됩니다.

**GitHub 가져오기·삭제를 쓰려면** gh CLI 로그인이 한 번 필요합니다 (안 해도 나머지 기능은 그대로 동작합니다):

```bash
gh auth login
```

잘 됐는지 확인하려면 `bash scripts/doctor.sh` 를 실행하세요.
...
| 선택 | 쓰임 |
|---|---|
| **GitHub CLI (`gh`)** 로그인 | **GitHub 에서 가져오기**(내 저장소 목록), 저장소 삭제, 카드의 **공개/비공개** 배지, "이름 변경" 이 GitHub 저장소 이름까지 바꿀 때. `gh auth login` 으로 로그인합니다. |
...
- **GitHub 에서 가져오기**: 헤더 `⬇ 가져오기` → 내 GitHub 저장소 목록에서 검색해 클론(gh 로그인 필요). 이미 받은 저장소는 목록에서 구분 표시. GitHub 주소를 직접 붙여넣어 가져올 수도 있습니다.
- **프로젝트 삭제**: 카드에서 삭제 → 로컬 폴더를 휴지통으로 이동. 원하면 **GitHub 저장소까지 함께 삭제**(체크박스, gh 로그인 필요). 실행 중인 프로젝트와 이 도구 자신의 폴더는 삭제할 수 없습니다.
- **메모**: 카드마다 한 줄 메모를 달아 어떤 프로젝트인지 표시 (`~/.local/state/project-launcher/notes.json` 에 저장).
