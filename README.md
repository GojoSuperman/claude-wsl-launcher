...
**대시보드 헤더의 `⟳ 업데이트` 버튼**을 누르면 끝입니다. 새 버전이 있는지 확인하고, 받고, 적용을 위해 서버를 끄는 것까지 안내해 줍니다. 그다음 바탕화면 단축키를 다시 실행하면 됩니다. 터미널은 필요 없습니다.

> 이 버튼이 헤더에 안 보인다면 버튼이 생기기 전 버전입니다. 아래 방법으로 **한 번만** 업데이트하면, 이후로는 버튼을 쓸 수 있습니다.

<details>
<summary>터미널로 업데이트하기 (버튼이 없는 버전이거나, 버튼이 실패했을 때)</summary>
...
- **런처 자체 업데이트**: 헤더 `⟳ 업데이트` → 새 버전 확인 후 받기(`git pull --ff-only` + `npm install`), 적용을 위한 서버 종료까지 한 흐름. 직접 고친 파일이 업데이트와 겹치거나 이력이 갈라졌으면 **아무것도 바꾸지 않고** 이유를 알려줍니다.
